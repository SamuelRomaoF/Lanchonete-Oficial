...
import { z } from "zod";
import { userTypeEnum, orderStatusEnum, paymentMethodEnum } from '@shared/schema';
import bcrypt from 'bcrypt';
import { ZodError } from 'zod';
import { fromZodError } from 'zod-validation-error';

// Usar exclusivamente o armazenamento JSON
const dataStorage = jsonStorage;
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/categories/:id', async (req, res) => {
    try {
      ...
      const category = await dataStorage.getCategory(id);
  ...
  app.post('/api/categories', adminMiddleware, async (req, res) => {
    try {
      ...
      const newCategory = await dataStorage.createCategory(result.data);
  ...
  app.put('/api/categories/:id', adminMiddleware, async (req, res) => {
    try {
      ...
      const updatedCategory = await dataStorage.updateCategory(id, result.data);
  ...
  app.get('/api/products', async (req, res) => {
    try {
      ...
      if (categoryId && !isNaN(categoryId)) {
        products = await dataStorage.getProductsByCategory(categoryId);
      } else {
        products = await dataStorage.getProducts();
      }
  ...
  app.get('/api/products/featured', async (req, res) => {
    try {
      const products = await dataStorage.getFeaturedProducts();
      return res.status(200).json(products);
    } catch (error) {
  ...
  app.get('/api/products/promotions', async (req, res) => {
    try {
      const products = await dataStorage.getPromotionProducts();
      return res.status(200).json(products);
    } catch (error) {
  ...
  app.post('/api/products', adminMiddleware, async (req, res) => {
    try {
      ...
      const newProduct = await dataStorage.createProduct(result.data);
  ...
  app.put('/api/products/:id', adminMiddleware, async (req, res) => {
    try {
      ...
      const updatedProduct = await dataStorage.updateProduct(id, result.data);
  ...
  app.delete('/api/products/:id', adminMiddleware, async (req, res) => {
    try {
      ...
      const success = await dataStorage.deleteProduct(id);
  ...
  app.get('/api/orders', adminMiddleware, async (req, res) => {
    try {
      const orders = await dataStorage.getOrders();
  ...
  app.get('/api/users/:userId/orders', authMiddleware, async (req, res) => {
    try {
      ...
      const orders = await dataStorage.getOrdersByUser(userId);
      return res.status(200).json(orders);
    } catch (error) {
  ...
  app.get('/api/orders/:id', authMiddleware, async (req, res) => {
    try {
      ...
      const orderDetails = await dataStorage.getOrderWithItems(id);
  ...
  app.post('/api/orders', authMiddleware, async (req, res) => {
    try {
      ...
      for (const item of items) {
        ...
        const product = await dataStorage.getProduct(item.productId);
      ...
      // Criar pedido
      const newOrder = await dataStorage.createOrder(orderResult.data, validItems);
  ...
  app.patch('/api/orders/:id/status', adminMiddleware, async (req, res) => {
    try {
      ...
      const updatedOrder = await dataStorage.updateOrderStatus(id, status);
  ...
  app.post('/api/payments', authMiddleware, async (req, res) => {
    try {
      ...
      const order = await dataStorage.getOrder(orderId);
      ...
      const existingPayment = await dataStorage.getPaymentByOrder(orderId);
      ...
      // Criar pagamento
      const newPayment = await dataStorage.createPayment(result.data);
      
      // Atualizar status do pedido para confirmado
      await dataStorage.updateOrderStatus(orderId, 'confirmado');
  ...
  app.get('/api/payments/:id', authMiddleware, async (req, res) => {
    try {
      ...
      const payment = await dataStorage.getPayment(id);
  ...
  app.get('/api/orders/:orderId/payment', authMiddleware, async (req, res) => {
    try {
      ...
      const payment = await dataStorage.getPaymentByOrder(orderId);
  ...
  app.patch('/api/payments/:id/status', adminMiddleware, async (req, res) => {
    try {
      ...
      const updatedPayment = await dataStorage.updatePaymentStatus(id, status);
      ...
      if (status === 'aprovado') {
        await dataStorage.updateOrderStatus(updatedPayment.orderId, 'preparo');
      }
      
      // Se o pagamento for recusado, atualizar status do pedido para cancelado
      if (status === 'recusado') {
        await dataStorage.updateOrderStatus(updatedPayment.orderId, 'cancelado');
      }
